...
app.post('/register', (req, res) => { /* Saves new user  
    const username = req.body.username;
    const password = req.body.myPassword;

    try {
        const fileContents = fs.readFileSync("./"+user+".txt", "utf-8");
...
app.post('/login', (req, res) => { /* Checks password  });
try {
    user = "auser";
    const fileContents = fs.readFileSync("./"+user+".txt", "utf-8");
    console.log(fileContents);
...
try {
    ...
    fs.writeFileSync("./"+user+".txt", activities);
}
catch {
    console.log("User file does not exist");
}
